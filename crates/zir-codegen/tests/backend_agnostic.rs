...
fn test_backend_factory() {
    ...
    assert!(backend.is_ok(), "Backend factory should succeed");
}

/// Test that the backend implements the trait correctly.
#[test]
fn test_backend_trait_implementation() {
    let backend = create_backend(CodegenConfig::default()).unwrap();
...
fn test_compile_const_function() {
    ...
    let mut backend = create_backend(CodegenConfig::default()).unwrap();
    let ir = compile_to_ir_text(backend.as_mut(), &body, sig_void_to_i64());

    assert!(ir.is_ok(), "Compilation should succeed");
...
fn test_codegen_test_case() {
    ...
    assert!(ir.is_ok(), "Test case should run successfully");
}
...
fn test_run_standard_tests() {
    ...
    assert!(results.is_ok(), "All standard tests should pass");
...
fn test_multiple_backend_instances() {
    ...
    let mut backend1 = create_backend(CodegenConfig::default()).unwrap();
    let mut backend2 = create_backend(CodegenConfig::default()).unwrap();

    let ir1 = compile_to_ir_text(backend1.as_mut(), &body, sig_void_to_i64()).unwrap();
    let ir2 = compile_to_ir_text(backend2.as_mut(), &body, sig_void_to_i64()).unwrap();
...
fn test_backend_config_independence() {
    ...
    let backend1 = create_backend(config1).unwrap();
    let backend2 = create_backend(config2).unwrap();
