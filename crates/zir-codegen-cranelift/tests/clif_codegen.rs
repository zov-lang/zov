...
fn compile_to_clif(body: &zir::mir::Body<'_>, sig: zir_codegen::FunctionSignature) -> String {
    let mut backend = create_backend(zir_codegen::CodegenConfig::default()).unwrap();
    compile_to_ir_text(backend.as_mut(), body, sig).unwrap()
}

#[test]
fn test_backend_name() {
    let backend = create_backend(zir_codegen::CodegenConfig::default()).unwrap();
...
fn test_standard_tests_all_pass() {
    ...
    let results = zir_codegen::testing::run_standard_tests(create_backend).unwrap();
